 ...
 * And with a division of logic and rendering it becomes easier to test.
 *
 * In this file we are looking at the logic of the Context and how to test it.
 * In the other test you can see how having a Context makes a rendering component that uses the Context as simple as our simplest test last week!
 *
 * So when looking at the Context(Provider) it is clear that we need to check a couple of things:
 * - There is an initialUser prop, we need to check that that is set and that the initial isLoggedIn is then correct!
 * - There is an `isLoggedIn` value, we need to check that that correctly gives true and false
 * - There is a login function, we need to check that that changes if the user isLoggedIn
 * - There is a logout function, we need to check that that actually logs out the user (by checking isLoggedIn again!)
 */

/**
 * You may now be wondering how you are going to test these values as the LoggedInUserContextProvider does not render anything.
...
function TestComponent({ userToLogin }) {
  ...
  /** You can make this as complicated as you want, make whatever value you want available in a way you want */
  return (
    <div>
      <div
